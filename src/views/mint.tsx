...
import {useBalance,useAllowance,useApprove} from '../hook/useTokenContract'
...
const MintPage = () => {
  ...
  const { state: stateForApprove, send: approve } = useApprove();
  ...
  useEffect(() => {
    ...
    if (stateForApprove) {
      stateForApprove.status === 'Exception' &&
        toast.error(stateForApprove.errorMessage);
      stateForApprove.status === 'Success' &&
  ...
  const mintNow = async () => {
    ...
    try {
      if (!active || !account) {
        ...
        toast.error('Not enough SLR to mint!');
        return;
    }
        let result;
      if(allowance?.lt(price?.mul(count))){
        result = await approve(contractAddress,price?.mul(count).mul(100));
        return;
      }
      if (whitelistMode) {
        ...
        result = await mintWhitelist(count, data.proof);
      } else {
        ...
        <div className='rounded-[40px] bg-[#020315] mt-[50px] md:mt-[70px] lg:mt-[50px] xl:mt-[70px] 2xl:mt-[2vw]'>
          <div className='overflow-hidden mx-[5vw] xl:mx-[25vw] 2xl:mx-[15vw] md:px-[100px] h-full pb-[40px] md:py-[55px] whiteAlpha bg-opacity-25 backdrop-blur-md rounded-[40px] font-extrabold text-center text-[30px] md:text-[36px] xl:text-[38.8px] 2xl:text-[2vw] uppercase flex flex-col lg:flex-row xl:flex-col 2xl:flex-row gap-0 lg:gap-[100px] xl:gap-0 2xl:justify-around items-center border-[2px] border-white border-opacity-50'>
            <div className='w-full flex flex-col 2xl:justify-center 2xl:flex-row items-center xl:block 2xl:w-fit'>
              <span className='2xl:order-2 w-full md:w-[220px] 2xl:w-[15vw] h-[150px] md:h-[220px] 2xl:h-[15vw] md:rounded-full bg-white bg-opacity-10 backdrop-blur-md border-b-[2px] border-b-white border-opacity-50 md:border-[2px] border-[#ffffff] overflow-hidden xl:hidden 2xl:block pt-[40px] md:pt-[80px] 2xl:pt-[5vw] mb-[20px]'>
                <img
                  alt='top_ags'
                  src={top_ags}
              ...
              <p className='text-[#ffffff] pt-[10px]'>
            ...
            <div className='md:px-0'>
              <div className='flex items-center justify-center gap-[50px] lg:gap-[30px] xl:gap-[80px] font-extrabold'>
                <button
                  className='w-[40px] md:w-[60px] xl:w-[80px] 2xl:w-[100px] h-[40px] md:h-[60px] xl:h-[80px] 2xl:h-[100px] flex justify-center items-center bg-white hover:bg-gradient-to-tr from-[#D95C6B] to-[#D95C6B] rounded-full text-black hover:text-white'
              ...
              <p className='lg:text-[32px] xl:text-[36px] 2xl:text-[50px]'>
                Total {ethers.utils.formatEther(price.mul(count))} SLR
              </p>
              ...
              <button
                ...
                onClick={() => mintNow()}
              >
                {allowance?.lt(price?.mul(count))?"Approve":"Mint Now"}
              </button>
            </div>
          </div>
        </div>
