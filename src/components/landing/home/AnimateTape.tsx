const getAnimateText = (white?: boolean) => {
  ...
  const append = (
    <>
      {line}
      <span>Solaris Finance</span>
      {line}
      <span>Earn UNIW + ETHW</span>
    </>
  );
  let append_text = [];
  for (let i = 0; i < 10; i++) {
...
const AnimateTape = ({ pink }: { pink?: boolean }) => {
  ...
  const pinkTape = (
    <div className='animate-tape lg:-rotate-[8deg] 2xl:-rotate-0 2xl:translate-y-0 lg:-translate-y-[40px] w-[8000px] md:w-[9000px] lg:w-[11000px] xl:w-[20000px] h-[32px] md:h-[48px] lg:h-[56px] xl:h-[74px] 2xl:h-[100px] bg-gradient-to-t from-[#121226] to-[#5480BE] text-white font-bold text-[18px] lg:text-[22px] xl:text-[28px] 2xl:text-[36px]'>
      {getAnimateText()}
    </div>
  );
